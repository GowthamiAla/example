...
export class HealthTablesComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.authService.solarHealthItems().subscribe(
      (data: any) => {
        ...
        console.log("healthItems SUCCESS")
      },
      error => {
      ...
      (data: any) => {
        ...
        console.log("calendar health SUCCESS")
      },
      error => {
        console.log(error.status)
        if (error.status == 401) {
        ...
        console.log("calendar health ERROR")
      });
  }
  ...
  public datachart(diskSpace: any) {
    ...
    this.remainingDiskSpace = Math.trunc(((diskSpace.total / 1073741824) - (diskSpace.free / 1073741824)) / ((diskSpace.total / 1073741824)) * 100);
