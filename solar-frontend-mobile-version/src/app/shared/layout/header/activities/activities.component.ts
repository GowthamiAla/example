...
export class ActivitiesComponent implements OnInit, OnDestroy {
    ...
    onEventNavigation(date) {
        ...
        this.active = false;
        dropdown.fadeOut();
        let goDate = date.split('T');
        let link = ['/calendar', { date: goDate[0] }];
    ...
    getNotificationCountAfterRead() {
        this.activitiesService.getNotificationCount().subscribe(
            data => {
                ...
                this.active = false;
                $('.ajax-dropdown', this.el.nativeElement).fadeOut()
                let link = ['/notifications/notificationslist', { data: 'notificationsList' }];
    ...
    calendarEventsList() {
        this.active = false;
        $('.ajax-dropdown', this.el.nativeElement).fadeOut()
        let link = ['/notifications/notificationslist', { data: 'calendarsList' }];
        this.router.navigate(link);
        // let link = ['/notifications/notificationslist'];
        // this.router.navigate(link);
    }
