...
export class LayoutService {
  ...
  //adding custom function to set default theme
  ...
  setCustomTheme() {
    ...
    if (this.store.mobileViewActivated) {
      this.store.fixedHeader=true;
      localStorage.setItem("sm-fixed-header","true");
    }else{
      this.store.fixedHeader=false;
      localStorage.setItem("sm-fixed-header","false");
    }
    this.onSmartSkin(glassSkinObject);
  }
}
