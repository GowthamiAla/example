...
export class DatatablesRestDemoComponent implements OnInit {
  ...
  ngOnInit() {
    this.authService.loggerItems().subscribe(
      (state: any) => {
        ...
        // });
        console.log("LoggerItems SUCCESS")
      },
      error => {
        // this.loginErrorMessgae = "Please Enter Valid Login Credentials.";
        //  this.loading = false;
        console.log("LoggerItems  ERROR")
      });
  ...
  updateLoggerEffectiveLevell(loggerLevels: any) {
    console.log(JSON.stringify(loggerLevels))
    this.authService.updateLoggerEffectiveLevell(loggerLevels).subscribe(
      data => {
        console.log("updateLoggerEffectiveLevell SUCCESS")
        // let link = ['/admin/datatables']
      ...
      error => {
        // this.loginErrorMessgae = "Please Enter Valid Login Credentials.";
        //  this.loading = false;
        console.log("updateLoggerEffectiveLevell ERROR")
      });
  }

  private extractData(res) {
    //  this.loggerLevels= JSON.parse(res._body).levels
    //   console.log( this.loggerLevels)
